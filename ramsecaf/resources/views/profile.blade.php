...
<body>
  ...
  <ol class="list-group list-group m-5">
    ...
    @if($pending->cart_status == "paid")
    
    @if($pending->id == $product[0]->cart_id)
    <li class="list-group-item d-flex justify-content-between align-items-start">
      <div class="ms-2">
        <a href="/order-summarry/{{$pending->id}}" class="text-dark">
          <h5 class="" class="row"><strong class="h3 fw-bold">{{$product[0]->store_name}} <span class="badge rounded-pill bg-warning"> Order # {{$pending->id}}</span></strong></h5>
          @forEach($product as $product)
          <h6 class="m-0 mx-3">{{$product->product_quantity." x ". $product->productname}}</h6>
          @endforEach
          <p style="font-style: italic; color: gray;">Expected pick up time: 12:15 pm</p>
